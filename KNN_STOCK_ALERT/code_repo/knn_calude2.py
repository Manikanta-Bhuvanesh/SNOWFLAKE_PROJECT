...
def calculate_supertrend(df, length=10, factor=3.0, ma_type='WMA'):
    """Ultra-optimized SuperTrend with maximum vectorization"""
    ...
    tr = np.where(np.isnan(prev_close), 
                  high - low,
                  np.maximum(np.maximum(high - low, np.abs(high - prev_close)), 
    ...
    # Optimized loop with pre-computed conditions
    prev_upper = np.concatenate([[np.nan], upper_band[:-1]])
    prev_lower = np.concatenate([[np.nan], lower_band[:-1]])
    prev_close_vals = np.concatenate([[np.nan], close[:-1]])
    
    for i in range(1, n):
        # Band adjustments
        if not (lower_band[i] > prev_lower[i] or prev_close_vals[i] < prev_lower[i]):
            final_lower[i] = prev_lower[i]
        
        if not (upper_band[i] < prev_upper[i] or prev_close_vals[i] > prev_upper[i]):
            final_upper[i] = prev_upper[i]
        
        # Direction logic
        if np.isnan(atr[i-1]):
            direction[i] = 1
        elif supertrend[i-1] == prev_upper[i]:
    ...
    # Assign to dataframe
    df['atr'] = atr
    df['upperBand'] = final_upper
    df['lowerBand'] = final_lower
